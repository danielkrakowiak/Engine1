...
GenerateMipmapWithSampleRejectionFragmentShader::GenerateMipmapWithSampleRejectionFragmentShader()
{}
...
void GenerateMipmapWithSampleRejectionFragmentShader::initialize( ComPtr< ID3D11Device >& device )
{
    { // Create sampler configuration.
        ...
        if ( result < 0 )
            throw std::exception( "GenerateMipmapWithSampleRejectionFragmentShader::compileFromFile - Failed to create texture sampler state." );
    }

    {
        ...
        if ( result < 0 )
            throw std::exception( "GenerateMipmapWithSampleRejectionFragmentShader::compileFromFile - creating constant buffer failed." );
    }
}
    ...
    const int srcMipLevel, const float maxAcceptableValue )
{
    ...
    if ( result < 0 ) throw std::exception( "GenerateMipmapWithSampleRejectionFragmentShader::setParameters - mapping constant buffer to CPU memory failed." );
    ...
    dataPtr->maxAcceptableValue      = maxAcceptableValue;
