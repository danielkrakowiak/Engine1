...
ResampleTextureFragmentShader::ResampleTextureFragmentShader() :
	m_resourceCount( 0 )
{}
...
void ResampleTextureFragmentShader::initialize( ComPtr< ID3D11Device3 >& device )
{
	{ // Create sampler configuration.
		...
		if ( result < 0 )
			throw std::exception( "GenerateMipmapFragmentShader::compileFromFile - Failed to create texture sampler state." );
	}

	{
		...
		if ( result < 0 )
			throw std::exception( "GenerateMipmapFragmentShader::compileFromFile - creating constant buffer failed." );*/
	}
}
	...
	const int srcMipLevel )
{
	...
	if ( result < 0 )
		throw std::exception( "GenerateMipmapFragmentShader::setParameters - mapping constant buffer to CPU memory failed." );
