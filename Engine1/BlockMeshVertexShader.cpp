...
void BlockMeshVertexShader::compileFromFile( std::string path, ID3D11Device& device )
{
	if ( m_compiled ) throw std::exception( "BlockMeshVertexShader::compileFromFile - Shader has already been compiled" );
	...
	{ //compile shader
		...
		if ( result < 0 ) {
			if ( errorMessage ) {
				...
				throw std::exception( ( std::string( "BlockMeshVertexShader::compileFromFile - Compilation failed with errors: " ) + compileMessage ).c_str() );
			} else {
				throw std::exception( "BlockMeshVertexShader::compileFromFile - Failed to open file" );
			}
		}
		...
		if ( result < 0 ) throw std::exception( "BlockMeshVertexShader::compileFromFile - Failed to create shader" );
	}

	{
		...
		if ( result < 0 ) throw std::exception( "BlockMeshVertexShader::compileFromFile - creating input layout failed" );
	}
...
{
	if ( !m_compiled ) throw std::exception( "BlockMeshVertexShader::setParameters - Shader hasn't been compiled yet" );

	D3D11_MAPPED_SUBRESOURCE mappedResource;
	ConstantBuffer* dataPtr;

	HRESULT result = deviceContext.Map( m_constantInputBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource );
	if ( result < 0 ) throw std::exception( "BlockMeshVertexShader::setParameters - mapping constant buffer to CPU memory failed" );
...
{
	if ( !m_compiled ) throw std::exception( "BlockMeshVertexShader::getInputLauout() - Shader hasn't been compiled yet." );
