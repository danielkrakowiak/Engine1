           ...
           uint  groupIndex : SV_GroupIndex )
{
    ...
    float newValue = 2.0f * skipPixelIfBelowValue + 1.0f;

    newValue = min( newValue, g_texture[ ( int2 )dispatchThreadId.xy + int2(  0, -1 ) ] ); // Top-neighbor.
    newValue = min( newValue, g_texture[ ( int2 )dispatchThreadId.xy + int2( -1,  0 ) ] ); // Left-neighbor.
    newValue = min( newValue, g_texture[ ( int2 )dispatchThreadId.xy + int2(  1,  0 ) ] ); // Right-neighbor.
    newValue = min( newValue, g_texture[ ( int2 )dispatchThreadId.xy + int2(  0,  1 ) ] ); // Bottom-neighbor.
