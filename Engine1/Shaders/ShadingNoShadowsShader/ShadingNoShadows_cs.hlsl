...
cbuffer ConstantBuffer : register( b0 )
{
    ...
    float  ambientOcclusionAvailable; // 1.0 if available, 0 otherwise.
    float3 pad3;
};
...
Texture2D<float>  g_ambientOcclusionTexture : register( t5 ); 
           ...
           uint  groupIndex : SV_GroupIndex )
{
    ...
    const float  surfaceAmbientOcclusion = max( 1.0 - ambientOcclusionAvailable, g_ambientOcclusionTexture[ dispatchThreadId.xy ] );
    ...
    for ( uint i = 0; i < pointLightCount; ++i )
    {
        ...
        float lightAttenuationFactor = 1 / (1 + lightLinearAttenuationFactor[ i ].x * distToLight + lightQuadraticAttenuationFactor[ i ].x * distToLight * distToLight );
        ...
        outputColor.rgb += calculateSurfaceLighting( pointLightColors[ i ].rgb * lightAttenuationFactor * surfaceAmbientOcclusion, surfaceNormal, dirToLight, dirToCamera,
